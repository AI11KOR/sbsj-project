...
class ChromaVectorStore:
    ...
    def __init__(
        ...
        """
        # 저장 경로 설정
        # - Windows에서 사용자 폴더 경로에 한글이 포함되면(예: C:\Users\김우현\...)
        #   Chroma(hnswlib)가 인덱스 파일(header)를 열지 못하는 케이스가 있습니다.
        # - 따라서 환경변수로 ASCII 경로로 우회할 수 있게 합니다.
        env_persist = os.getenv("CHROMA_PERSIST_DIRECTORY") or os.getenv("CHROMA_DB_DIR")
        if persist_directory is None:
            if env_persist:
                persist_directory = env_persist
            else:
                # 현재 파일 기준 상대 경로로 data/chroma_db 설정
                current_dir = Path(__file__).parent.parent
                persist_directory = str(current_dir / "data" / "chroma_db")

        # 상대경로가 들어오면 절대경로로 정규화
        ...
        print("[DB]  ChromaDB 초기화 중...")
        print(f"   - 저장 경로: {persist_directory}")
        print(f"   - 컬렉션: {collection_name}")

        # ChromaDB 클라이언트 생성 (영구 저장)
        # - 일부 환경에서 posthog/telemetry 의존성 버전 충돌로 경고 로그가 발생할 수 있습니다.
        # - 기능에는 영향이 거의 없지만 로그가 지저분해져 기본값으로 텔레메트리를 끕니다.
        #   필요 시 CHROMA_ANONYMIZED_TELEMETRY=true 로 켤 수 있습니다.
        enable_telemetry = os.getenv("CHROMA_ANONYMIZED_TELEMETRY", "false").strip().lower() in (
            "1",
            "true",
            "yes",
            "y",
        ...
        self.client = chromadb.PersistentClient(
            ...
            settings=Settings(anonymized_telemetry=enable_telemetry),
        )
