...
# import 시점에 바로 실행돼서, lightgbm이 없으면 앱이 바로 종료되는 문제 해결
def safe_joblib_load(path: str):
    try:
        return joblib.load(path)
    except Exception as e:
        print(f"[MODEL LOAD FAIL] {path}: {e}")
        return None
...
models = {
    "cafe": safe_joblib_load("models/lgb_2025_model_cafe.pkl"),
    "hansic": safe_joblib_load("models/lgb_2025_model_hansic.pkl"),
    "hof": safe_joblib_load("models/lgb_2025_model_hof.pkl"),
}
...
def predict_custom():
    try:
        ...
        model = models.get(shop_type)
        if model is None:
            return jsonify({"error": f"{shop_type} 모델이 로드되지 않았습니다. 서버 로그를 확인하세요."}), 500
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=5000, debug=False) # 배포 환경에서는 debug=False로 설정
